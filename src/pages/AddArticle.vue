...
import { mdOption, articleObj } from "../types/article";
...
export default class AddArticle extends Vue {
  ...
  handleClose(id: string): void {
    var index: number = this.dynamicTags.findIndex(
      (item: tagObj) => item._id == id
    );
    if (index >= 0) this.dynamicTags.splice(index, 1);
  }
  ...
  async handleInputConfirm(): Promise<any> {
    var colors: Array<string> = ["success", "info", "warning", "danger", ""];
    var max: number = colors.length - 1;
    var type: string = colors[Math.round(Math.random() * max)];
  ...
  chooseTag(id: string) {
    var tag: tagObj|undefined = this.tagData.find(
      (item: tagObj) => item._id == id
    );
    var index: number =this.dynamicTags.findIndex((item: tagObj) => item._id == tag!._id);
  ...
  async save(): Promise<any> {
    ...
    this.article.context.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, function(
      match,
      capture
    ):any{
      if (
        imglist.indexOf(capture) == -1 &&
        capture.indexOf(globalConfig.serverUrl) != -1
      )
    ...
    if (this.isUpdate == false) {
      ...
      if (res.code == 200) this.$router.push("/article");
    } else {
  ...
  saveToSession(): void {
    this.initArticle();
    var article: string = JSON.stringify(this.article);
  ...
  autoSave(): void {
    this.timer = setInterval(() => {
      ...
      if (context != "") this.saveToSession();
    }, 120000);
  }
  async checkType(): Promise<any> {
    if (this.$route.query.article_id) {
      ...
      if (!data._id) return this.$router.push("/notarticle");
      this.updateId = _id;
      this.article = {
        title,
        context,
  ...
  async handleEditorImgAdd(pos:number, $file:any) {
    ...
    if (res.url) {
      ...
      let content:string = this.article.mdValue;
      ...
      if (content.includes(name)) {
        ...
        let insertStr:Function = (soure:string, start:number, newStr:string):string => {
  ...
  handleEditorImgDel(pos:any): void {
