  ...
  <div class="chat">
    ...
    <div class="card">
      <div class="heads">
        ...
        <el-date-picker v-model="queryDate" type="date" placeholder="选择日期" @change="search"></el-date-picker>
      ...
      >
        ...
        <el-table-column prop="chatName" label="留言人" width="180px"></el-table-column>
        <el-table-column prop="chatContent" label="内容"></el-table-column>
        <el-table-column prop="chatWay" label="联系方式" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{scope.row.chatWay=='：'?'暂无':scope.row.chatWay}}</span>
          </template>
        </el-table-column>
...
import { getChats, delChats } from "../network/chats";
import { chatObj } from "../types/chats";
@Component
export default class Chats extends Vue {
  multipleSelection: Array<chatObj> = [];
  chatData: Array<chatObj> = [];
  queryPeople: string = "";
  queryDate: any = "";
  pageNum: number = 1;
  pageSzie: number = 10;
  total: number = 0;
  handleSelectionChange(val: Array<chatObj>): void {
