    ...
    <div class="card">
      ...
      <div class="lists">
        ...
        <div
          ...
          :key="pic._id"
          @click="$router.push({path:'/picturelist',query:{id:pic._id}})"
        >
          ...
          <img
            class="picture"
            :src="pic.cover==''?require('../assets/img/timg.jpg'):pic.cover"
            :title="pic.createTime|dateFormat"
          />
          <div class="title">{{pic.title}}</div>
          <div class="count">{{pic.count}}</div>
          <div class="tag" :style="{'borderColor':pic.tagColor}" v-if="pic.tag!=''">
...
import { picObj, picInfo } from "../types/picture";
...
export default class Picture extends Vue {
  ...
  rules: any = {
    ...
    tag: [{ min: 1, max: 2, message: "长度在 1 到 2个字符", trigger: "change" }]
  };
  pictureList: Array<picObj> = [];
  handleClose(): void {
    (this.$refs as any).addForm.resetFields();
  }
  setPicture(): void {
    if (this.pictureList.length > 0) this.wantDel = !this.wantDel;
  }
  active(e: any): void {
    e.currentTarget.children[0].className = "el-icon-delete active";
  }
  unActtive(e: any): void {
    e.currentTarget.children[0].className = "el-icon-delete";
  }
  delPictures(pic: picObj): void {
    if (pic.count == 0)
  ...
  sureAdd(): void {
    (this.$refs as any)["addForm"].validate(async (valid: boolean) => {
      if (valid) {
