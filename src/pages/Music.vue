<template>
  <div class="music">
    ...
    <div class="card">
      ...
      <el-table stripe border :data="songList" style="width: 100%">
        <el-table-column prop="title" label="歌名" show-overflow-tooltip></el-table-column>
        <el-table-column prop="artist" label="歌手" show-overflow-tooltip></el-table-column>

        <el-table-column prop="cloudID" label="网易云ID" width="150px"></el-table-column>
        <el-table-column prop="lrc" label="歌词" width="80px">
          <template slot-scope="scope">{{scope.row.lrc==''?'暂无歌词':'歌词完整'}}</template>
        </el-table-column>
        <el-table-column label="操作" width="120px">
          <template slot-scope="scope">
            <el-button
              ...
              icon="el-icon-delete"
              size="small"
              @click="delSong(scope.row._id)"
            >删除音乐</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <el-dialog title="查询结果" :visible.sync="dialogVisible" width="300px">
      <div class="songitem">
        <span>歌名：</span>
        <span>{{this.MusicForm.title}}</span>
      </div>
      <div class="songitem">
        <span>歌手：</span>
        <span>{{this.MusicForm.artist}}</span>
      </div>
      <div class="songitem">
        <span>资源：</span>
        <br />
        <span>{{this.MusicForm.src}}</span>
      </div>
      <div class="songitem">
        <span>歌词：</span>
        <span>{{this.MusicForm.lrc==''?'暂无歌词':'歌词完整'}}</span>
      </div>
...
export default class Music extends Vue {
  ...
  async search(): Promise<any> {
    if (this.musicId.trim() != "")
      Promise.all([
        ...
        if (music && music.code == 200) this.MusicForm.src = music.data[0].url;
  ...
  async getSong(): Promise<any> {
    const res = await getSongs();
    if (res.code == 200) this.songList = res.data;
  }
  delSong(id: string): void {
    this.$confirm(`确认删除吗？`)
      .then(async _ => {
