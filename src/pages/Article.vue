  ...
  <div class="article">
    ...
    <div class="card">
      ...
      >
        ...
        <el-table-column prop="tagList" label="标签" width="320px">
          <template slot-scope="scope">
            <el-tag
        ...
        <el-table-column prop="contextText" label="概要" show-overflow-tooltip>
          <template slot-scope="scope">
            <span class="context_text">{{scope.row.contextText.slice(0,20)}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="star" label="点赞数" width="120px">
          <template slot-scope="scope">
            <span>
        ...
        <el-table-column prop="createTime" label="发表日期" width="180px">
          <template slot-scope="scope">
...
export default class Article extends Vue {
  query: string = "";
  articleData: Array<articleObj> = [];
  tagSelectList: Array<tagObj> = [];
  pageNum: number = 1;
  pageSzie: number = 5;
  total: number = 0;
  tagquery: string = "";
  multipleSelection: Array<articleObj> = [];
  handleSelectionChange(val: Array<articleObj>): void {
  ...
  delarts(): void {
    ...
    this.multipleSelection.forEach((art: articleObj) => {
      list.push(art._id!);
    });
    if (list.length > 0)
  ...
  async toTop(article: articleObj): Promise<any> {
    var type: number = article.isTop == 1 ? 0 : 1;
    const res = await TopArticle(article._id!, type);
    if (res.code == 200) this.getArticle();
  }
  created(): void {
    if (this.$route.params.tagId) this.tagquery = this.$route.params.tagId;
    this.getArticle();
    this.getTag();
  }
}
